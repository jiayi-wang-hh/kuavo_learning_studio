...
def process_qwen38(
    ...
    """Qwen3.8 video processing with explicit FPS metadata."""
    try:
        from qwen_vl_utils import process_vision_info
    except ImportError as exc:
        raise ImportError(
            "qwen-vl-utils is required for Qwen3.8 video inference."
        ) from exc
    ...
    # Qwen's official Qwen3-VL path returns both decoded videos and their
    # metadata. Passing video_metadata is necessary for accurate FPS sampling.
...
def prepare_inputs(
    ...
    messages = build_messages(
        ...
        prompt=prompt,
        fps=args.fps,
        # qwen-vl-utils expects file:// URIs and supplies accurate video
        # metadata to the Qwen3.8 processor.
        qwen38_native=False,
    )
