#!/usr/bin/env python3
"""
Evaluate a simple V1 "no progress toward basket" failure trigger.
...
- once activated, monitor progress over a temporal window;
- if distance-to-nearest-basket does not improve enough for long enough,
  emit PAUSE;
...
def parse_args():
    ...
    p.add_argument(
        ...
        type=float,
        default=0.02,
        help="Toy displacement from its initial reliable position required to activate monitoring.",
    )
    p.add_argument(
        "--progress-window-s",
        type=float,
        default=1.5,
        help="Lookback window used to measure progress toward nearest basket.",
    )
    p.add_argument(
        "--min-progress",
        type=float,
        default=0.02,
        help="Required reduction in normalized toy-to-basket distance over the window.",
    )
    p.add_argument(
        "--pause-after-s",
        type=float,
        default=2.0,
        help="Continuous no-progress duration required before PAUSE.",
    )
    p.add_argument(
        "--pregrasp-timeout-s",
        type=float,
    ...
    p.add_argument(
        ...
        default=0.008,
        help=(
            "Optional extra stagnation cue: toy displacement over progress window "
            "below this value counts as no-progress. The trigger still also checks "
            "basket-distance progress."
        ),
    )
    p.add_argument(
        "--use-motion-cue",
        ...
        default=False,
        help=(
            "If enabled, no-progress requires BOTH insufficient basket progress "
            "and low toy motion. Default False follows the user's simpler rule."
        ),
    )
...
def process_rollout(
    ...
    for frame_id, fr in frames.items():
        ...
        for side, toy_cls in (("left", "left_toy"), ("right", "right_toy")):
            ...
            if st["done"]:
                ...
                insufficient_progress = progress < args.min_progress

                if args.use_motion_cue:
                    low_motion = toy_motion < args.min_toy_motion
                    no_progress_now = insufficient_progress and low_motion
                else:
                    no_progress_now = insufficient_progress
                ...
                if st["no_progress_s"] >= args.pause_after_s:
                    raw_trigger = True
                    reason = (
                        f"no_progress: progress={progress:.4f}, "
                        f"toy_motion={toy_motion:.4f}, "
                        f"timer={st['no_progress_s']:.2f}s"
                    )
                else:
                    reason = (
                        f"monitoring: progress={progress:.4f}, "
                        f"toy_motion={toy_motion:.4f}, "
                        f"timer={st['no_progress_s']:.2f}s"
                    )
        ...
        rows.append(
            {
                ...
                "left_no_progress_s": f"{side_outputs['left']['no_progress_s']:.3f}",
                "right_no_progress_s": f"{side_outputs['right']['no_progress_s']:.3f}",
    ...
    summary = {
        ...
        "params": {
            ...
            "use_motion_cue": args.use_motion_cue,
            "min_toy_motion": args.min_toy_motion,
