#!/usr/bin/env python3
"""
Evaluate a simple V4 pregrasp-timeout + post-activation stagnation trigger with basket-bbox DONE detection.
...
- basket geometry is used for DONE detection (toy center inside an expanded basket bbox);
  basket distance is kept only as a diagnostic, not as a monotonic-progress requirement;
...
def parse_args():
    ...
    p.add_argument(
        ...
        default=0.10,
        help=(
            "Fallback nearest-basket center distance for DONE. "
            "Primary DONE test is toy-center inside expanded basket bbox."
        ),
    )
    p.add_argument(
        "--basket-done-margin",
        ...
        default=0.02,
        help=(
            "Normalized margin added around each basket bbox for DONE detection. "
            "Example: 0.02 = 2%% of image width/height."
        ),
    )
...
def load_tracking_csv(path: Path):
    ...
    with path.open("r", newline="", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            if cls in TOY_CLASSES:
                ...
                # CSV bbox coordinates are pixel coordinates. Normalize them using
                # the center relation if possible. Since side_center_x/y are already
                # normalized, infer frame size from bbox center / normalized center.
                px_cx = 0.5 * (x1 + x2)
                px_cy = 0.5 * (y1 + y2)

                frame_w = px_cx / cx if cx > 1e-8 else None
                frame_h = px_cy / cy if cy > 1e-8 else None

                if frame_w and frame_h and frame_w > 1 and frame_h > 1:
                    box_norm = (
                        x1 / frame_w,
                        y1 / frame_h,
                        x2 / frame_w,
                ...
                frames[frame]["basket_detections"].append(
                    {"center": (cx, cy), "box": box_norm}
                )
                basket_rows.append(
                    {"center": (cx, cy), "box": box_norm}
                )
    ...
    # Baskets are static. Split detections by x around the global median,
    # then estimate one robust center + bbox per basket.
    xs = [r["center"][0] for r in basket_rows]
    split = statistics.median(xs)

    left = [r for r in basket_rows if r["center"][0] <= split]
    right = [r for r in basket_rows if r["center"][0] > split]

    if not left or not right:
        basket_rows_sorted = sorted(
            basket_rows,
    ...
    def robust_basket(cluster):
        centers = [r["center"] for r in cluster]
        center = (
            statistics.median([p[0] for p in centers]),
            statistics.median([p[1] for p in centers]),
        )

        valid_boxes = [r["box"] for r in cluster if r["box"] is not None]

        if valid_boxes:
            box = (
                statistics.median([b[0] for b in valid_boxes]),
                statistics.median([b[1] for b in valid_boxes]),
                statistics.median([b[2] for b in valid_boxes]),
                statistics.median([b[3] for b in valid_boxes]),
            )
        else:
            box = None
    ...
    baskets = [robust_basket(left)]

    if right:
        baskets.append(robust_basket(right))

    return dict(sorted(frames.items())), baskets
...
def nearest_basket_distance(
    ...
    ds = [dist_xy(toy_xy, b["center"]) for b in baskets]
...
def toy_is_done(
    toy_xy: Tuple[float, float],
    baskets: List[dict],
    basket_done_margin: float,
    success_distance: float,
) -> Tuple[bool, Optional[int], str]:
    # Primary rule: toy center is inside an expanded basket bbox.
    for i, basket in enumerate(baskets):
        if point_inside_expanded_box(
            ...
            return True, i, "inside_basket_bbox"

    # Fallback if bbox is unavailable or detection is noisy.
    d, idx = nearest_basket_distance(toy_xy, baskets)
    if d <= success_distance:
        return True, idx, "near_basket_center"
...
def process_rollout(
    ...
    for frame_id, fr in frames.items():
        ...
        for side, toy_cls in (("left", "left_toy"), ("right", "right_toy")):
            ...
            # Primary criterion: toy center enters an expanded basket bbox.
            # Fallback: nearest-basket center distance.
            is_done_now, done_basket_idx, done_reason = toy_is_done(
                toy_xy,
                baskets,
                args.basket_done_margin,
    ...
    summary = {
        ...
        "params": {
            ...
            "basket_done_margin": args.basket_done_margin,
