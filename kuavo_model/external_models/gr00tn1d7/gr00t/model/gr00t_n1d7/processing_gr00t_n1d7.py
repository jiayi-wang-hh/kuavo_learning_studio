...
def _resolve_to_local_path(model_name: str) -> str:
    """Resolve a HuggingFace model name to a local cache path if available.

    Using a local path avoids gated-repo download errors and the
    ``is_base_mistral`` API call that requires network access.
    """
    ...
    hub_model_dir = os.path.join(cache_dir, "models--" + model_name.replace("/", "--"))
    snapshots_dir = os.path.join(hub_model_dir, "snapshots")
    if os.path.isdir(snapshots_dir):
        snapshots = sorted(os.listdir(snapshots_dir))
        if snapshots:
            return os.path.join(snapshots_dir, snapshots[-1])

    # Fallback: keep the original name (will trigger network access)
    return model_name
...
def build_processor(model_name: str, transformers_loading_kwargs: dict) -> Qwen3VLProcessor:
    ...
    resolved_path = _resolve_to_local_path(model_name)
    return Qwen3VLProcessor.from_pretrained(resolved_path, **transformers_loading_kwargs)
